...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    wcex.lpszClassName = L"Romanchuk HW_8";
...
HRESULT CreateCubeResources() {
    ...
    ImGui_ImplWin32_Init(FindWindow(L"Romanchuk HW_8", L"Romanchuk HW_8"));
...
void RenderScene(bool renderToBackbuffer) {
    ...
    RECT rc;
    GetClientRect(FindWindow(L"Romanchuk HW_8", L"Romanchuk HW_8"), &rc);
...
void CleanupDevice() {
    ...
    if (g_pCullingCS) { g_pCullingCS->Release(); g_pCullingCS = nullptr; }
    if (g_pCullingParamsBuffer) { g_pCullingParamsBuffer->Release(); g_pCullingParamsBuffer = nullptr; }
    if (g_pIndirectArgsBuffer) { g_pIndirectArgsBuffer->Release(); g_pIndirectArgsBuffer = nullptr; }
    if (g_pIndirectArgsUAV) { g_pIndirectArgsUAV->Release(); g_pIndirectArgsUAV = nullptr; }
    if (g_pObjectsIdsBuffer) { g_pObjectsIdsBuffer->Release(); g_pObjectsIdsBuffer = nullptr; }
    if (g_pObjectsIdsUAV) { g_pObjectsIdsUAV->Release(); g_pObjectsIdsUAV = nullptr; }
    if (g_pIndirectArgsStagingBuffer) { g_pIndirectArgsStagingBuffer->Release(); g_pIndirectArgsStagingBuffer = nullptr; }
    if (g_pSceneCBBuffer) { g_pSceneCBBuffer->Release(); g_pSceneCBBuffer = nullptr; }

    if (g_pFullScreenLayout) { g_pFullScreenLayout->Release(); g_pFullScreenLayout = nullptr; }
    if (g_pFullScreenVB) { g_pFullScreenVB->Release(); g_pFullScreenVB = nullptr; }
    if (g_pPostProcessPS) { g_pPostProcessPS->Release(); g_pPostProcessPS = nullptr; }
    if (g_pPostProcessVS) { g_pPostProcessVS->Release(); g_pPostProcessVS = nullptr; }
    if (g_pPostProcessSRV) { g_pPostProcessSRV->Release(); g_pPostProcessSRV = nullptr; }
    if (g_pPostProcessRTV) { g_pPostProcessRTV->Release(); g_pPostProcessRTV = nullptr; }
    if (g_pPostProcessTex) { g_pPostProcessTex->Release(); g_pPostProcessTex = nullptr; }
