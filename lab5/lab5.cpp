...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    wcex.lpszClassName = L"Romanchuk HW_5";
...
void Render() {
    ...
    RECT rc;
    GetClientRect(FindWindow(L"Romanchuk HW_5", L"Romanchuk HW_5"), &rc);
    ...
    g_greenAnim += 0.02f;
    float greenOffsetY = 2.0f * sinf(g_greenAnim);
    XMMATRIX modelgreen = XMMatrixTranslation(2.0f, 0.0f, greenOffsetY);
    XMVECTOR greenPos = XMVectorSet(2.0f, 0.0f, greenOffsetY, 1.0f);
    ...
    if (pinkDist >= greenDist) {
        ...
        m_pDeviceContext->UpdateSubresource(g_pTransparentBuffer, 0, nullptr, &redColor, 0, 0);
        m_pDeviceContext->Draw(ARRAYSIZE(g_CubeVertices), 0);

        XMMATRIX modelgreenT = XMMatrixTranspose(modelgreen);
        m_pDeviceContext->UpdateSubresource(g_pModelBuffer, 0, nullptr, &modelgreenT, 0, 0);
        m_pDeviceContext->UpdateSubresource(g_pTransparentBuffer, 0, nullptr, &greenColor, 0, 0);
        m_pDeviceContext->Draw(ARRAYSIZE(g_CubeVertices), 0);
    } else {
        XMMATRIX modelgreenT = XMMatrixTranspose(modelgreen);
        m_pDeviceContext->UpdateSubresource(g_pModelBuffer, 0, nullptr, &modelgreenT, 0, 0);
